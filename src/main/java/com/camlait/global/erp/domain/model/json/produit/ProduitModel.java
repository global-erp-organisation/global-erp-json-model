...
public class ProduitModel extends Entite {
    ...
    public static ProduitModel createProduitModel(Produit p) {
        return builder().categorieId(p.getCategorie() == null ? null : p.getCategorie().getCategorieProduitId()).codeProduit(p.getCodeProduit())
                .dateDeCreation(p.getDateDeCreation()).derniereMiseAJour(p.getDerniereMiseAJour()).descriptionProduit(p.getDescriptionProduit())
                .prixUnitaireMarge(p.getPrixUnitaireMarge()).prixUnitaireProduit(p.getPrixUnitaireProduit()).produitId(p.getProduitId())
                .produitTaxable(p.isProduitTaxable()).produitTaxeModels(getTaxes(p)).suiviEnStock(p.isSuiviEnStock()).build();

    }

    private static Collection<ProduitTaxeModel> getTaxes(Produit p) {
        return p.getProduitTaxes().stream().map(t -> {
    ...
    public ProduitModel(Long produitId, String codeProduit, String descriptionProduit, double prixUnitaireProduit, double prixUnitaireMarge,
                        Long categorieId, boolean produitTaxable, Collection<ProduitTaxeModel> produitTaxeModels, Date dateDeCreation,
                        Date derniereMiseAJour, boolean suiviEnStock) {
        super();
